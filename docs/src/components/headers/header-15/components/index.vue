<template>
  <div ref="wrap" class="wrap-container">
    <div
      ref="realBox"
      :style="pop"
      @mouseenter="onEnter"
    ...
    >
      ...
      <div :style="float" v-html="state.copyHtml"></div>
    </div>
  </div>
</template>
...
let propsData = computed(()=>props.data||[])
...
const pop = computed(()=>{
  if(isHorizontal.value){
    console.log(state.value.realBoxWidth,'state.value.realBoxWidth')
...
function init (){
  nextTick(()=>{
    ...
    if(isHorizontal.value){
      ...
      let slotListWidth = slotList.value?.offsetWidth || state.value.width
...
// 数据量过大预警
function dataWarm (){
  if(propsData.value.length>100){
    console.warn(`数据达到了${propsData.value.length}条有点多哦~,可能会造成部分老旧浏览器卡顿。`)
  }
}
